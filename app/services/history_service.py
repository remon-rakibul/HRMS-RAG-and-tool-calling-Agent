...
class HistoryService:
    ...
    def get_or_create_thread(
        ...
        # Since thread_id is globally unique, check by thread_id first
        thread = self.db.query(ChatThread).filter(
            ChatThread.thread_id == thread_id
        ).first()
        
        if thread:
            # Thread exists - return it (can continue chatting)
            return thread
        ...
        try:
            ...
            # If creation fails (e.g., race condition - thread was created by another request),
    ...
    def get_thread_messages(
        ...
        # Since thread_id is globally unique and represents employee_id,
        # we just need to check if the thread exists (not which user created it)
        thread = self.db.query(ChatThread).filter(
            ChatThread.thread_id == thread_id
        ).first()
