"""HRMS Admin Leave Approval Tool.

This tool allows admins to approve leave requests for employees by searching for them by name
and matching leave requests by applied date. It handles the complete leave approval workflow
through 9 HRMS API endpoints.
"""
...
def approve_leave_for_employee(
    ...
    Examples:
    - "approve leave for neha muquid applied on 2026-01-12"
    - "approve leave request for john doe dated 2026-01-15"
    """
    # Set default remarks
    if remarks is None:
        remarks = "Approved"
    ...
    print("\n" + "="*70, flush=True)
    print("[HRMS Admin] ADMIN LEAVE APPROVAL PROCESS STARTED", flush=True)
    print("="*70, flush=True)
    ...
    # Step 1: Get Branch
    print("[HRMS Admin] Step 1/9: GetBranchExtension...", flush=True)
    try:
        resp1 = httpx.get(
            f"{HRMS_BASE_URL}/api/ws/controlPanelService/GetBranchExtension",
            params={"flag": 7, "ComId": 3, "OrgId": 5},
            headers=headers,
            timeout=30.0,
            verify=False
        )
        ...
        print(f"[HRMS Admin] ⚠️ Step 1 error: {str(e)}", flush=True)
        # Continue even if branch call fails
    
    # Step 2: Get All Employees and Search by Name
    print("[HRMS Admin] Step 2/9: GetEmployeeServiceData (searching for employee)...", flush=True)
    ...
    # Step 3: Get Employee Leave Requests
    print("[HRMS Admin] Step 3/9: GetEmployeeLeaveRequests (finding matching leave request)...", flush=True)
    try:
        resp3 = httpx.get(
        ...
        if resp3.status_code != 200:
            return f"❌ Failed to retrieve leave requests. HTTP {resp3.status_code}"
        
        # Parse leave requests
        try:
            leave_data = resp3.json()
        ...
        if len(leave_list) == 0:
            ...
            return f"❌ No leave requests found for {found_name}. Please check if the employee has any pending leave requests."
        ...
        applied_from_date = _extract_date_from_datetime(
            matched_leave.get("appliedFromDate") or 
            matched_leave.get("AppliedFromDate") or ""
        )
        applied_to_date = _extract_date_from_datetime(
            matched_leave.get("appliedToDate") or 
            matched_leave.get("AppliedToDate") or ""
        )
    ...
    try:
        # Step 4: Get Employee Leave Request By Id
        print("[HRMS Admin] Step 4/9: GetEmployeeLeaveRequestById...", flush=True)
        resp4 = httpx.get(
            f"{HRMS_BASE_URL}/api/hrms/leave/LeaveRequest/GetEmployeeLeaveRequestById",
            params={
                "employeeId": employee_id,
        ...
        # Step 5: Get Total Request Days
        print("[HRMS Admin] Step 5/9: GetTotalRequestDays...", flush=True)
        resp5 = httpx.get(
            ...
            params={
                ...
                "appliedFromDate": applied_from_date,
        ...
        # Step 6: Employee Leave Balance
        print("[HRMS Admin] Step 6/9: GetLeaveBalance...", flush=True)
        resp6 = httpx.get(
        ...
        print("[HRMS Admin] Step 7/9: LeaveRequestApproval (approving leave request)...", flush=True)
        resp7 = httpx.post(
            f"{HRMS_BASE_URL}/api/hrms/Leave/LeaveRequest/LeaveRequestApproval",
            json={
                "employeeId": employee_id,
                "employeeLeaveRequestId": employee_leave_request_id,
                "remarks": remarks,
        ...
        if resp7.status_code != 200:
            error_text = resp7.text[:500] if resp7.text else "No response body"
            return f"❌ Failed to approve leave request. HTTP {resp7.status_code}: {error_text}"
        
        # Parse approval response
        approval_result = resp7.json() if resp7.text else {}
        ...
        if not approval_status:
            ...
            return f"❌ Leave approval failed: {error_msg}"
        
        print(f"[HRMS Admin] ✓ Leave request approved successfully", flush=True)
        
        # Step 8: Send Leave Email
        print("[HRMS Admin] Step 8/9: SendLeaveEmail...", flush=True)
        resp8 = httpx.post(
            f"{HRMS_BASE_URL}/api/hrms/leave/LeaveRequest/SendLeaveEmail",
            json=approval_result,  # Use response from Step 7
            headers=headers,
            timeout=30.0,
            verify=False
        )
        ...
        # Step 9: Get Employee Leave Requests (Final Check)
        print("[HRMS Admin] Step 9/9: GetEmployeeLeaveRequests (final verification)...", flush=True)
        resp9 = httpx.get(
            f"{HRMS_BASE_URL}/api/HRMS/Leave/LeaveRequest/GetEmployeeLeaveRequests",
            params={
                "month": 0,
        ...
        print("="*70, flush=True)
        print("[HRMS Admin] ✅ LEAVE APPROVAL PROCESS COMPLETED", flush=True)
        print("="*70 + "\n", flush=True)
        
        # Build success message
        success_msg = (
            f"✅ Leave approved successfully for {found_name}!\n"
        ...
        return f"❌ Network error during leave approval: {str(e)}"
    except Exception as e:
        error_msg = f"❌ Error during leave approval: {str(e)}"
