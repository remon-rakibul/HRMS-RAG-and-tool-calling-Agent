"""HRMS Admin Attendance Cancellation Tool.

This tool allows admins to cancel manual attendance requests for employees by searching for them by name
and matching attendance requests by applied date and time request type. It handles the complete attendance
cancellation workflow through 6 HRMS API endpoints.
"""
...
def cancel_attendance_for_employee(
    ...
    Examples:
    - "cancel attendance for neha muquid applied on 2026-01-12 for in-time"
    - "cancel attendance request for john doe dated 2026-01-15 for both times"
    """
    # Set default remarks
    if remarks is None:
        remarks = "Cancelled"
    ...
    print("\n" + "="*70, flush=True)
    print("[HRMS Admin] ADMIN ATTENDANCE CANCELLATION PROCESS STARTED", flush=True)
    print("="*70, flush=True)
    ...
    try:
        ...
        # Step 4: Cancel Manual Attendance
        print("[HRMS Admin] Step 4/6: CancelRequest (cancelling attendance request)...", flush=True)
        resp4 = httpx.post(
            f"{HRMS_BASE_URL}/api/hrms/Attendance/ManualAttendance/CancelRequest",
            json={
        ...
        if resp4.status_code != 200:
            ...
            return f"❌ Failed to cancel attendance request. HTTP {resp4.status_code}: {error_text}"
        
        # Parse cancellation response
        cancel_result = resp4.json() if resp4.text else {}
        ...
        if not cancel_status:
            ...
            return f"❌ Attendance cancellation failed: {error_msg}"
        
        print(f"[HRMS Admin] ✓ Attendance request cancelled successfully", flush=True)
        
        # Step 5: Send Email
        print("[HRMS Admin] Step 5/6: SendEmail (sending cancellation email)...", flush=True)
        resp5 = httpx.post(
            f"{HRMS_BASE_URL}/api/hrms/Attendance/ManualAttendance/SendEmail",
            json=cancel_result,  # Use response from Step 4
            headers=headers,
            timeout=30.0,
            verify=False
        )
        ...
        print("="*70, flush=True)
        print("[HRMS Admin] ✅ ATTENDANCE CANCELLATION PROCESS COMPLETED", flush=True)
        print("="*70 + "\n", flush=True)
        
        # Build success message
        success_msg = (
            f"✅ Attendance cancelled successfully for {found_name}!\n"
        ...
        return f"❌ Network error during attendance cancellation: {str(e)}"
    except Exception as e:
        error_msg = f"❌ Error during attendance cancellation: {str(e)}"
